...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif ($ajaxtp === 'delete') {
        ...
        try {
            $sql_delete = "DELETE FROM bg.materia_boletim WHERE mate_bole_cod = :mate_cod";
            $stmt = $pdo->prepare($sql_delete);
            $stmt->execute(['mate_cod' => $mate_cod]);
            $linhas_afetadas = $stmt->rowCount();
            registrarLog("Tentativa de exclusão: mate_bole_cod=$mate_cod. Linhas afetadas: $linhas_afetadas");

            if ($linhas_afetadas > 0) {
                registrarLog("Exclusão bem-sucedida para mate_bole_cod=$mate_cod.");
                respostaJSON('success', 'Exclusão bem-sucedida');
            } else {
                registrarLog("Nenhuma linha excluída para mate_bole_cod=$mate_cod.");
                respostaJSON('error', 'Nenhuma linha foi excluída. Verifique se o código está correto.');
            }
        } catch (PDOException $e) {
