...
if ($bole_cod > 0) {
    ...
    if (!$boletim) {
        ...
        header('Location: cad_assina_boletim_dp.php');
        exit;
    }
} else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['assinar_boletim'])) {
    ...
    if ($bole_cod_post > 0) {
        try {
            ...
            header('Location: cad_assina_boletim_dp.php?bole_cod=' . $bole_cod_post);
            exit;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['assinar_boletim'])) {
    try {
        ...
        // Redirecionar para a página com o parâmetro 'bole_cod'
        header('Location: cad_assina_boletim_dp.php?bole_cod=' . $bole_cod);
        exit;
    } catch (PDOException $e) {
        $_SESSION['message'] = ['type' => 'danger', 'text' => 'Erro ao processar a solicitação: ' . htmlspecialchars($e->getMessage())];
        header('Location: cad_assina_boletim_dp.php' . ($bole_cod > 0 ? '?bole_cod=' . $bole_cod : ''));
        exit;
    }
}
...
<body>
    <div class="container">
        ...
        <form method="POST" action="">
            ...
            <?php if ($bole_cod > 0): ?>
                <iframe id="pdfIframe" src="/boletim/pdf/boletim_pdf.php?bole_cod=<?= htmlspecialchars($bole_cod) ?>" title="Boletim PDF"></iframe>
            <?php endif; ?>
        </form>

        <!-- Modal de Confirmação para Assinar Boletim -->
        <?php if ($bole_cod > 0 && $boletim['bole_ass_dp'] != 1): ?>
            <div class="modal fade" id="confirmAssinarModal" tabindex="-1" aria-labelledby="confirmAssinarModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <form method="POST" action="cad_assina_boletim_dp.php">
